...
void WebserviceController::sendRequestPUT(const QString &urlStr, const QJsonObject &obj, DataType type)
{
    m_currentType = type;
...
void WebserviceController::sendRequestDEL(const QString &urlStr)
{
    qDebug() << "send request DELETE to server:\n" << urlStr;
    QUrl url(urlStr);
    QNetworkRequest request;
    request.setUrl(url);
...
void WebserviceController::onRecvReply(QNetworkReply *reply)
{
    ...
    } else if (m_currentType == REPLY) {
        qDebug() << "recv server reply";
        emit sigRecvServerReply();
    }
}
